...
def render_svg(
    ...
    for index, (language, _byte_count, percent) in enumerate(summary):
        ...
        x = 20 + column * 195
        percentage_x = 390 if sparse_layout else x + 177
        y = 95 + row * 22
        safe_name = html.escape(compact_name(language, 31 if sparse_layout else 13))
        rows.append(
            f'<rect x="{x}" y="{y - 10}" width="10" height="10" rx="2" fill="{color}"/>'
            f'<text x="{x + 17}" y="{y}" font-size="15" fill="#FFE2AE">{safe_name}</text>'
            f'<text x="{percentage_x}" y="{y}" text-anchor="end" font-size="14" fill="#D59D53">{percent:.2f}%</text>'
        )

    safe_user = html.escape(user)
    language_word = "language" if total_language_count == 1 else "languages"
    repository_word = "repository" if repository_count == 1 else "repositories"
    aria_label = html.escape(
        f"{total_language_count} {language_word} across {repository_count} public {repository_word} for {user}. "
...
<defs><filter id="g" x="-25%" y="-70%" width="150%" height="240%"><feGaussianBlur stdDeviation="1.1" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter><pattern id="s" width="4" height="4" patternUnits="userSpaceOnUse"><rect width="4" height="1" fill="#000" opacity=".3"/></pattern><radialGradient id="v"><stop offset="60%" stop-color="#000" stop-opacity="0"/><stop offset="100%" stop-color="#000" stop-opacity=".64"/></radialGradient><clipPath id="bar"><rect x="20" y="58" width="370" height="10" rx="5"/></clipPath></defs>
<rect width="410" height="196" rx="4" fill="#0D0804"/>
<rect width="410" height="32" rx="4" fill="#1A1007"/>
<text x="18" y="22" font-size="14" font-weight="700" fill="#FFB000" filter="url(#g)">{total_language_count} LANGUAGES</text>
<text x="392" y="22" text-anchor="end" font-size="10.5" fill="#A67637">PUBLIC · DPOWCOIN EXCLUDED</text>
<text x="20" y="49" font-size="13" fill="#D59D53">most-used by GitHub language bytes</text>
<g clip-path="url(#bar)"><rect x="20" y="58" width="370" height="10" fill="#2F1F0E"/>{''.join(segments)}</g>
{''.join(rows)}
<path d="M20 178H390" stroke="#4E3517"/>
<text x="20" y="191" font-size="10.5" fill="#A67637">{repository_count} PUBLIC NON-FORK REPOS · REPOSITORY-SCOPED TOKEN</text>
<rect width="410" height="196" rx="4" fill="url(#s)"/><rect width="410" height="196" rx="4" fill="url(#v)"/><rect x=".5" y=".5" width="409" height="195" rx="4" fill="none" stroke="#7A511B"/>
</svg>
'''
    ET.fromstring(svg)
    return svg
